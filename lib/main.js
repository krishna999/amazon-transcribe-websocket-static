...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            transcript = transcript.replace("full stop.", ".");
            transcript = transcript.replace(". Full Stop", ".");
            transcript = transcript.replace("Stop", ".");
            transcript = transcript.replace("new paragraph","\r");
            transcript = transcript.replace("New paragraph","\r");
            transcript = transcript.replace("New Paragraph","\r");
            transcript = transcript.replace("New line", "\r");
            
            
            transcript = transcript.replace("Numeral one.", "1.");
             transcript = transcript.replace("numeral one.", "1.");
...
function createPresignedUrl() {
    ...
    return v4.createPresignedURL(
        ...
        crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
            ...
            'expires': 15,
            'region': region,
            'query': "language-code=" + languageCode + "&media-encoding=pcm&sample-rate=" + sampleRate + "&type=DICTATION&specialty=RADIOLOGY"
        }
    );
}
